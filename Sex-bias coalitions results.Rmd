...
```{r compute contrasts for Models 4 & 5}
## philopatry
p_muFemalebiased_PhilopatryB<- post_m4$b_muFemalebiased_Intercept+post_m4$b_muFemalebiased_PhilopatryB
p_muFemalebiased_PhilopatryF<- post_m4$b_muFemalebiased_Intercept+post_m4$b_muFemalebiased_PhilopatryF
p_muFemalebiased_PhilopatryM<- post_m4$b_muFemalebiased_Intercept+post_m4$b_muFemalebiased_PhilopatryM
# F vs all others: concatenate samples and draw 8000 random samples
# still not 100% sure if this is correct - does it matter that samples are no longer matched?
## actually, the current approach assumes that all categories occur at the same rate
# e.g. if cat1 2x more likely than cat2, and 10x more likely than cat3, combining samples will yield sth like 6x more likely - but what if cat2 was much more common than cat3? then re-coding cat2+cat3 as cat_non1 will give a different result

# alternative would be to average samples across the non-F categories
p_muFemalebiased_PhilopatryAnyNonFlong<- c(post_m4$b_muFemalebiased_Intercept, p_muFemalebiased_PhilopatryM, p_muFemalebiased_PhilopatryB) # combine samples from all non F 
p_muFemalebiased_PhilopatryAnyNonF<- sample(p_muFemalebiased_PhilopatryAnyNonFlong, size = length(p_muFemalebiased_PhilopatryF), replace = FALSE) # draw random samples from the combined posterior, to get same number of samples as for F

ORfemalebiasphiloFvsB<- exp(p_muFemalebiased_PhilopatryF/p_muFemalebiased_PhilopatryB)
ORfemalebiasphiloFvsM<- exp(p_muFemalebiased_PhilopatryF/p_muFemalebiased_PhilopatryM)
ORfemalebiasphiloFvsAnyNonF<- exp(p_muFemalebiased_PhilopatryF/p_muFemalebiased_PhilopatryAnyNonF)

p_muMalebiased_PhilopatryB<- post_m4$b_muMalebiased_Intercept+post_m4$b_muMalebiased_PhilopatryB
p_muMalebiased_PhilopatryF<- post_m4$b_muMalebiased_Intercept+post_m4$b_muMalebiased_PhilopatryF
p_muMalebiased_PhilopatryM<- post_m4$b_muMalebiased_Intercept+post_m4$b_muMalebiased_PhilopatryM
p_muMalebiased_PhilopatryAnyNonMlong<- c(post_m4$b_muMalebiased_Intercept, p_muMalebiased_PhilopatryF, p_muMalebiased_PhilopatryB) # combine samples from all non M
p_muMalebiased_PhilopatryAnyNonM<- sample(p_muMalebiased_PhilopatryAnyNonMlong, size = length(p_muMalebiased_PhilopatryM), replace = FALSE) # draw random samples from the combined posterior, to get same number of samples as for F

ORmalebiasphiloMvsB<- exp(p_muMalebiased_PhilopatryM/p_muMalebiased_PhilopatryB)
ORmalebiasphiloMvsF<- exp(p_muMalebiased_PhilopatryM/p_muMalebiased_PhilopatryF)
ORmalebiasphiloMvsAnyNonM<- exp(p_muMalebiased_PhilopatryM/p_muMalebiased_PhilopatryAnyNonM)

```

To test our socio-ecological predictors we ran one binomial model on the presence of female coalitions (Model 4), including food defensibility (yes/no) and female philopatry (yes/no) as predictors, and one on the presence of male coalitions (Model 5), including sexual dimorphism and male philopatry (yes/no) as predictors. Note that for the purpose of these analyses we scored female philopatry when females or both sexes are philopatric, and male philopatry when males or both sexes are philopatric. As a robustness check, we also modeled each of these competing causes on its own. 

The predicted probabilities of coalition types as a function of covariates are shown in Figure X. In weak support of our philopatry prediction, the probability of female-biased coalitions was somewhat greater in female philopatric species (n = `r length(d$Philopatry[d$Philopatry=="F"])`), compared to species where both sexes disperse (n = `length(d$Philopatry[d$Philopatry=="N"])`, OR = `r round(median(exp(post_m4$b_muFemalebiased_PhilopatryF)),2)`, 90% CI = `r round(HPDI(exp(post_m4$b_muFemalebiased_PhilopatryF), prob=0.90)[1],2)` - `r round(HPDI(exp(post_m4$b_muFemalebiased_PhilopatryF), prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(post_m4$b_muFemalebiased_PhilopatryF>0)/nrow(post_m4),0)`%), where males are philopatric (n = `length(d$Philopatry[d$Philopatry=="M"])`, OR = `r round(median(ORfemalebiasphiloFvsM),2)`, 90% CI = `r round(HPDI(ORfemalebiasphiloFvsM, prob=0.90)[1],2)` - `r round(HPDI(ORfemalebiasphiloFvsM, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORfemalebiasphiloFvsM>1)/length(ORfemalebiasphiloFvsM),0)`%), where both sexes are philopatric (n = `length(d$Philopatry[d$Philopatry=="B"])`, OR = `r round(median(ORfemalebiasphiloFvsB),2)`, 90% CI = `r round(HPDI(ORfemalebiasphiloFvsB, prob=0.90)[1],2)` - `r round(HPDI(ORfemalebiasphiloFvsB, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORfemalebiasphiloFvsB>1)/length(ORfemalebiasphiloFvsB),0)`%), or compared to any of these categories (OR = `r round(median(ORfemalebiasphiloFvsAnyNonF),2)`, 90% CI = `r round(HPDI(ORfemalebiasphiloFvsAnyNonF, prob=0.90)[1],2)` - `r round(HPDI(ORfemalebiasphiloFvsAnyNonF, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORfemalebiasphiloFvsAnyNonF>1)/length(ORfemalebiasphiloFvsAnyNonF),0)`%), though there was high uncertainty in these estimates. In contrast, male coalitions were not more likely in species with male philopatry, whether compared to species in which both sexes disperse (OR = `r round(median(exp(post_m4$b_muMalebiased_PhilopatryM)),2)`, 90% CI = `r round(HPDI(exp(post_m4$b_muMalebiased_PhilopatryM), prob=0.90)[1],2)` - `r round(HPDI(exp(post_m4$b_muMalebiased_PhilopatryM), prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(post_m4$b_muMalebiased_PhilopatryM>0)/nrow(post_m4),0)`%), where females are philopatric (OR = `r round(median(ORmalebiasphiloMvsF),2)`, 90% CI = `r round(HPDI(ORmalebiasphiloMvsF, prob=0.90)[1],2)` - `r round(HPDI(ORmalebiasphiloMvsF, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORmalebiasphiloMvsF>1)/length(ORmalebiasphiloMvsF),0)`%), where both sexes are philopatric (OR = `r round(median(ORmalebiasphiloMvsB),2)`, 90% CI = `r round(HPDI(ORmalebiasphiloMvsB, prob=0.90)[1],2)` - `r round(HPDI(ORmalebiasphiloMvsB, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORmalebiasphiloMvsB>1)/length(ORmalebiasphiloMvsB),0)`%), or compared to all of these categories (OR = `r round(median(ORmalebiasphiloMvsAnyNonM),2)`, 90% CI = `r round(HPDI(ORmalebiasphiloMvsAnyNonM, prob=0.90)[1],2)` - `r round(HPDI(ORmalebiasphiloMvsAnyNonM, prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(ORmalebiasphiloMvsAnyNonM>1)/length(ORmalebiasphiloMvsAnyNonM),0)`%). 

With regard to the other predictions, female coalitions were not appreciably more likely in species with defensible food resources compared to species with non-defensible food resources (OR = `r round(median(exp(post_m4$b_muFemalebiased_Food_resource_defendable)),2)`, 90% CI = `r round(HPDI(exp(post_m4$b_muFemalebiased_Food_resource_defendable), prob=0.90)[1],2)` - `r round(HPDI(exp(post_m4$b_muFemalebiased_Food_resource_defendable), prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(post_m4$b_muFemalebiased_Food_resource_defendable>0)/nrow(post_m4),0)`%); however, as predicted the probability of male bias did increase with increasing sexual dimorphism (OR for 1SD change in dimorphism = `r round(median(exp(post_m4$b_muMalebiased_SexDim.z)),2)`, 90% CI = `r round(HPDI(exp(post_m4$b_muMalebiased_SexDim.z), prob=0.90)[1],2)` - `r round(HPDI(exp(post_m4$b_muMalebiased_SexDim.z), prob=0.90)[2],2)`, probability OR>1 = `r round(100*sum(post_m4$b_muMalebiased_SexDim.z>0)/nrow(post_m4),0)`%). Other associations that we did not make specific predictions for, i.e. between female coalitions and sexual dimorphism or between male coalitions and food defensibility were weak and uncertain (see supplement). These inferences did not change when considering each predictor in a model on its own (see Supplement).
